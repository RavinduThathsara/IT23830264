...
def get_output_fallback(text):
    try:
        url = "https://www.pixelssuite.com/api/transliterate"
...
# Clear previous input/output before each test
                try:
                ...
                # Wait for output and capture it robustly
                actual_output = ""
                tries = max(1, int(args.retries))

                for i in range(tries):
                    # Directly read the second textarea (Sinhala output box)
                    try:
                        current = page.locator("textarea").nth(1).input_value().strip()
                    except Exception:
                ...
                # Last fallback: JavaScript read of all textarea values
                # Try to read output from UI
                if not actual_output:
                    try:
                        values = page.evaluate("""
                            () => Array.from(document.querySelectorAll('textarea'))
                                .map(t => t.value || t.innerText || t.textContent || '')
                        """)
                        if len(values) > 1 and values[1].strip():
                            actual_output = str(values[1]).strip()
                    except Exception:
                        pass

                # If still empty → mark clearly
                if not actual_output:
                    actual_output = "NO OUTPUT (UI/API ERROR)"
